...
int PatternFinder::findCuda(int nb, deviceStubs* d_stubs, cudaStream_t* stream){
  ...
  */
  cudaSetHitsWrapper(d_stubs,nb,d_detector, stream);
      ...
      getHitsArray(d_detector,test_array,nb);
      
      for(int i=0;i<NB_LAYER*SIZE_LAYER*MAX_NB_STUBS_PER_SSTRIPS;i++){
	if(test_array[i]!=-1){
	  cout<<i<<" : "<<test_array[i]<<endl;
	}
      }
  ...
  resetDetector(d_detector, stream);
      ...
      if(res!=0 && time<min_time){
	min_b=b;
	min_t=t;
	min_time=time;
      }
      cout<<"Meilleure combinaison : "<<min_b<<" blocks et "<<min_t<<" threads en "<<min_time<<" ms"<<endl;
